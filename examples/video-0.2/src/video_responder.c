...
void prepare_response(packedobjectsdObject *pod_obj, char *movie_title, double max_price);
...
{ 
  ...
  while(1)
	...
	exit(EXIT_FAILURE);
      ...
      /* process search request to retrieve search details */
      if((ret = process_search(pod_obj, doc_search, "/video/message/search")) == -1) {
