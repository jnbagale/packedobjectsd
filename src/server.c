...
static pid_t fork_new_broker(char *address, int port_in, int port_out)
{
...
static serverObject *create_new_broker(serverObject *server_obj, char *request, char *buffer, int *buffer_size) {
...
void *start_server(void *server_object)
{
  ...
  while (1) {
	...
	if(buffer_size < 0) {
	  ...
	  if(node_type == PUBLISHER) {
	    /* Check if the process with the given process id is currently running in the system */
